...
5. **Native Method Libraries**
   * 실제 운영체제 의존적인 네이티브 코드 라이브러리 모음.
   * JNI를 통해 실행 엔진이 호출해서 사용한다.
...
| ------ | ------------------ | ----------------------- |
| 실행 방식  | 한 줄씩 즉시 해석 후 실행    | 런타임에 바이트코드를 네이티브 코드로 변환 |
| 속도     | 초기 실행 빠름, 전체 실행 느림 | 초기 지연 있으나 전체 실행 빠름      |
| 최적화    | ❌ 없음               | ✅ 있음 (인라이닝, 캐싱 등)       |
| 메모리 사용 | 적음                 | 더 많이 사용 (기계어 캐싱 때문)     |
