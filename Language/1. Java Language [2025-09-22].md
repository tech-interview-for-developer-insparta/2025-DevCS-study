...
## Heap 영역과 참조
- 자바에서 객체(`new Object()`)는 **Heap 영역**에 저장된다.  
...
- 그래서 Heap을 **Young 영역**과 **Old 영역**으로 나눠 관리한다.
...
## 추천 이미지 검색 키워드
구글 이미지에서 아래 키워드로 검색하면 시각적으로 이해하기 쉽다.

1. `JVM Heap Structure GC`  
2. `Stop The World GC illustration`  
3. `Mark and Sweep Garbage Collection`  
4. `Generational Garbage Collection Java`
```
